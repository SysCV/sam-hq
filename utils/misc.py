...
def masks_sample_points(masks,k=10):
    ...
    # y_mask = y_mask.flatten(1)
    # x_idx, idx = x_mask.sort(-1,True)

    # center = ((x_idx>0).sum(-1)/2).int().tolist()
    # x_center = x_idx[torch.arange(len(x_idx)).tolist(),center]
    # corre_idx =  idx[torch.arange(len(x_idx)).tolist(),center]
    # y_center = y_mask[torch.arange(len(y_mask)).tolist(),corre_idx.tolist()]


def masks_sample_neg_points(masks,k=10):
    """Sample points on mask
    """
    if masks.numel() == 0:
        return torch.zeros((0, 2), device=masks.device)
    ...
    for b_i in range(len(masks)):
        ...
        select_mask = (masks[b_i]>128).float()

        x_min, y_min, x_max, y_max = boxes[b_i].int()
        box_mask = torch.zeros((h,w)).to(select_mask)
        box_mask[y_min:y_max,x_min:x_max] = 1

        select_mask_new = torch.logical_xor(select_mask,box_mask)

        # debug = True
        # if debug:
        #     mask1 = select_mask.cpu().byte().numpy()*255
        #     mask2 = box_mask.cpu().byte().numpy()*255
        #     mask3 = select_mask_new.cpu().byte().numpy()*255
        #     cv2.imwrite('mask1.png',mask1)
        #     cv2.imwrite('mask2.png',mask2)
        #     cv2.imwrite('mask3.png',mask3)

        select_mask = select_mask_new
...
#     :param n_points_to_select: number of points to select
#     :param subsample_size: size of subsample to use for K-medoids
#     :return: tensor of shape (n_points_to_select, 2) containing the selected points
#     """
#     if mask.sum() == 0:
#         print(f"Warning: mask.sum() == 0, stack trace: {traceback.format_stack()}")
#         return torch.zeros((n_points_to_select, 2))

#     mask_pixels = mask.nonzero().float().cpu()
...
# def masks_sample_center_points(masks,k=1):
#     h, w = masks.shape[-2:]
#     samples = []
#     for b_i in range(len(masks)):
#         select_mask = (masks[b_i]>128)
#         samples_xy = extract_kmedoid_points(select_mask,k)
#         samples_xy = samples_xy.to(masks)
#         samples.append(samples_xy)
...
def masks_noise(masks):
    def get_incoherent_mask(input_masks, sfact):
        ...
        return mask_residue
    # gt_masks_vector = F.interpolate(target_masks, (256, 256), mode=‘bilinear’)
...
def mask_iou(pred_label,label):
    '''
    https://github.com/open-mmlab/mmsegmentation/blob/70477d21adfb4caaf2e35c8e6d88b9ee740486d6/mmseg/core/evaluation/metrics.py#L79
    '''
